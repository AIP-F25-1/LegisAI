...
st.set_page_config(page_title="LegisAI – Reasoning & Self-Consistency", layout="centered")
st.title("⚖️ LegisAI – Reasoning & Self-Consistency (CrewAI)")
...
if st.button("Run Cross-Consistency"):
    ...
    for i, pa in enumerate(res["per_agent"], 1):
        with st.expander(f"Agent {i}: {pa.get('verdict','N/A')}"):
            st.json(pa)
